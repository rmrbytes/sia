...
llm_repo_id = os.getenv("LLM_MODEL_NAME")
llm_model_dir = os.path.join(os.getenv("DATA_DIR", "data"), "models", llm_repo_id)
...
if llm_repo_id:
    snapshot_download(
        repo_id=llm_repo_id,
        local_dir=llm_model_dir,  # Ensure it's downloaded to /data/models
        token=token
)
